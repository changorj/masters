...
%              Series representation of a saw tooth wave.
%
...
t = 0:0.01:2;				% time over which we'll plot signal

N = [1 5 10 50 100];			% +/- values at which we'll truncate FS

for i = 1:5,				% compute truncated FS for above N values
   
   f = D0*ones(size(t));		% start out with DC bias term
   
   for n = N(i),			% loop over negative n
   ...
   hold on;
   plot([0 1 1 2 2 2],[0 1 0 1 0 2],'r');
   hold off;
   xlabel('t ');
   ylabel('f(t)');
